...
class WilmerMikadoElementorProductInfo extends \Elementor\Widget_Base {
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="mkdf-pi-title entry-title" ' . wilmer_mikado_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="mkdf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . wilmer_mikado_get_inline_style( $product_info_style ) . '>';
...
\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new WilmerMikadoElementorProductInfo() );