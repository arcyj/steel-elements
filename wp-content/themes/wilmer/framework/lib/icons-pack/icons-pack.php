...
foreach ( glob( MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( wilmer_mikado_is_customizer_item_enabled( $module_load, 'wilmer_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'wilmer_mikado_get_icon_pack_collections' ) ) {
	function wilmer_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'wilmer_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'wilmer_mikado_get_icon_pack_options' ) ) {
	function wilmer_mikado_get_icon_pack_options() {
		$options = apply_filters( 'wilmer_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'wilmer_mikado_get_icon_pack_social_options' ) ) {
	function wilmer_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'wilmer_mikado_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: WilmerMikadoClassIconCollections
 */
class WilmerMikadoClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = wilmer_mikado_get_icon_pack_collections();
		$this->iconPackOptions       = wilmer_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = wilmer_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = wilmer_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'wilmer' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'wilmer' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'wilmer' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'wilmer' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'wilmer' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'wilmer' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'wilmer' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'wilmer' )       => 'fa-lg',
			esc_html__( 'Small', 'wilmer' )      => 'fa-2x',
			esc_html__( 'Medium', 'wilmer' )     => 'fa-3x',
			esc_html__( 'Large', 'wilmer' )      => 'fa-4x',
			esc_html__( 'Very Large', 'wilmer' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iWilmerMikadoInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'wilmer' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'wilmer' ) ] = '';
    ...
    public function getElementorParamsArray($control_object, $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false){
        ...
        $control_object->add_control(
            'icon_pack', [
                'label' => esc_html__( 'Icon Pack', 'wilmer' ),
