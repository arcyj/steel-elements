<?php
namespace WilmerCore\CPT\Shortcodes\ImageGallery;
...
class ImageGallery implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Image Gallery', 'wilmer-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							'heading'     => esc_html__( 'Gallery Type', 'wilmer-core' ),
							'value'       => array(
								esc_html__( 'Image Grid', 'wilmer-core' ) => 'grid',
								esc_html__( 'Masonry', 'wilmer-core' )    => 'masonry',
								esc_html__( 'Slider', 'wilmer-core' )     => 'slider',
								esc_html__( 'Carousel', 'wilmer-core' )   => 'carousel'
							),
							'save_always' => true,
							'admin_label' => true
						),
                        ...
                        array(
                            'type'        => 'dropdown',
                            'param_name'  => 'slider_overflow',
                            'heading'     => esc_html__( 'Enable slider overflow', 'wilmer-core' ),
                            'description' => esc_html__('Enabling this options will make rest of your slides visible out of the boundaries of slider', 'galatia'),
                            'value'       => array_flip( wilmer_mikado_get_yes_no_select_array( false, false ) ),
                            'save_always' => true,
                            'dependency'  => array( 'element' => 'type', 'value' => array( 'carousel', 'slider' ) ),
                            'group'       => esc_html__( 'Slider Settings', 'wilmer-core' )
                        ),
						array(
							'type'        => 'dropdown',
						...
						array(
							...
							'heading'     => esc_html__( 'Enable Slider Padding', 'wilmer-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = wilmer_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'image-gallery', '', $params );
