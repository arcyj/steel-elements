...
class FleiACFAdminColumns
{
    ...
    {
        // ACF related
        add_action('acf/init', [$this, 'action_add_acf_actions']); // add ACF fields
        add_action('admin_enqueue_scripts', [$this, 'action_enqueue_admin_scripts']);

        // WP admin post and taxonomy index tables ("All posts" screens)
        add_action('parse_request', [$this, 'action_prepare_columns'], 10);
        add_action('pre_get_terms', [$this, 'action_prepare_columns'], 10);

        add_action('pre_get_posts', [$this, 'action_prepare_query_sort']);
    }
    ...
    public function action_enqueue_admin_scripts()
    {
        ...
        if ($screen && $screen->id == 'acf-field-group') { // only hook on ACF field editor page
            wp_enqueue_script(self::ACF_SETTING_NAME, plugins_url('main.js', __FILE__), ['acf-field-group'], null, true);
        }
    }
    ...
    public function action_prepare_columns()
    {
        ...
        foreach ($field_groups as $fgroup) {
            ...
            foreach ($fgroup_fields as $field) {
                ...
                var_dump(self::COLUMN_NAME_PREFIX . $field['name']);
        ...
        if (! empty($this->admin_columns)) {
            if ($is_post_type_index) {
                add_filter('manage_' . $screen->post_type . '_posts_columns', [$this, 'filter_manage_posts_columns']); // creates the columns
                add_filter('manage_' . $screen->id . '_sortable_columns', [$this, 'filter_manage_sortable_columns']); // make columns sortable
                add_action('manage_' . $screen->post_type . '_posts_custom_column', [$this, 'action_manage_posts_custom_column'], 10, 2); // outputs the columns values for each post
            } elseif ($is_taxonomy_index) {
                add_filter('manage_edit-' . $screen->taxonomy . '_columns', [$this, 'filter_manage_posts_columns']); // creates the columns
                add_filter('manage_' . $screen->taxonomy . '_custom_column', [$this, 'filter_manage_taxonomy_custom_column'], 10, 3); // outputs the columns values for each post
            }
        }
    }
    ...
    public function action_prepare_query_sort($query)
    {
        if ($this->is_acf_active() && $this->is_valid_admin_screen() && $query->query_vars && isset($query->query_vars['orderby'])) {
            ...
            if (array_key_exists($orderby, $this->admin_columns)) {
                ...
                $meta_query = [
                    'relation' => 'OR',
                    ['key' => $this->get_clean_column($orderby), 'compare' => 'NOT EXISTS'], // 'NOT EXISTS' needs to go first for proper sorting
