...
class WilmerInstagramWidget extends WP_Widget {
	protected $params;
	
	public function __construct() {
		parent::__construct(
			'mkdf_instagram_widget',
			esc_html__( 'Wilmer Instagram Widget', 'wilmer-instagram-feed' ),
			array(
				'description' => esc_html__( 'Display your Instagram feed', 'wilmer-instagram-feed' )
			)
		);
	...
	protected function setParams() {
		$this->params = array(
			array(
				'name'  => 'title',
				'type'  => 'textfield',
				'title' => esc_html__( 'Title', 'wilmer-instagram-feed' )
			),
			array(
				'name'    => 'type',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Type', 'wilmer-instagram-feed' ),
				'options' => array(
			...
			array(
				'name'    => 'number_of_cols',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Number of Columns', 'wilmer-instagram-feed' ),
				'options' => array(
					'2' => esc_html__( 'Two', 'wilmer-instagram-feed' ),
					'3' => esc_html__( 'Three', 'wilmer-instagram-feed' ),
					'4' => esc_html__( 'Four', 'wilmer-instagram-feed' ),
					'6' => esc_html__( 'Six', 'wilmer-instagram-feed' ),
					'9' => esc_html__( 'Nine', 'wilmer-instagram-feed' )
				)
			),
			array(
				'name'  => 'number_of_photos',
				'type'  => 'textfield',
				'title' => esc_html__( 'Number of Photos', 'wilmer-instagram-feed' )
			),
			array(
				'name'    => 'space_between_items',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Space Between Items', 'wilmer-instagram-feed' ),
				'options' => wilmer_mikado_get_space_between_items_array( false, array( 'large', 'huge' ) )
			),
			array(
				'name'    => 'show_instagram_icon',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Instagram Icon', 'wilmer-instagram-feed' ),
				'options' => wilmer_mikado_get_yes_no_select_array( false, true )
			),
			array(
				'name'  => 'transient_time',
				'type'  => 'textfield',
				'title' => esc_html__( 'Images Cache Time', 'wilmer-instagram-feed' )
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( is_array( $images_array ) && count( $images_array ) ) { ?>
			<ul class="mkdf-instagram-feed clearfix mkdf-col-<?php echo esc_attr( $number_of_cols ); ?> <?php echo esc_attr( $widget_class ); ?>" <?php echo wilmer_mikado_get_inline_attrs( $slider_data ); ?>>
				<?php
...
if ( ! function_exists( 'wilmer_instagram_feed_widget_load' ) ) {
	function wilmer_instagram_feed_widget_load() {
		if ( wilmer_instagram_theme_installed() ) {
			register_widget( 'WilmerInstagramWidget' );
		}
	}
	
	add_action( 'widgets_init', 'wilmer_instagram_feed_widget_load' );
}